...
Attributes Vertex(Attributes input, uint vertexID : SV_VertexID, uint instanceID : SV_InstanceID)
{
    input.position.xyz = 0;
    for (uint i = 0; i < _CageVertCount; ++i)
    {
        input.position.xyz +=
            _Weights[vertexID * _CageVertCount + i]
            *
            _ControlPoints[instanceID * _CageVertCount + i];
    }
...
{
    // Vertex inputs
    uint flags = _Flags[input[0].instanceID];
    bool flip = flags & 1 != 0;
    int ia = 0;
    int ib = flip ? 2 : 1;
    int ic = flip ? 1 : 2;

    float3 wp0 = input[ia].position.xyz;
    float3 wp1 = input[ib].position.xyz;
    float3 wp2 = input[ic].position.xyz;

    float2 uv0 = input[ia].texcoord;
    float2 uv1 = input[ib].texcoord;
    float2 uv2 = input[ic].texcoord;

    // Cap triangle
    float3 wn = ConstructNormal(wp0, wp1, wp2);
    float np = 1;
    float3 wn0 = lerp(input[ia].normal, wn, np);
    float3 wn1 = lerp(input[ib].normal, wn, np);
    float3 wn2 = lerp(input[ic].normal, wn, np);
    outStream.Append(VertexOutput(wp0, wn0, input[ia].tangent, uv0));
    outStream.Append(VertexOutput(wp1, wn1, input[ib].tangent, uv1));
    outStream.Append(VertexOutput(wp2, wn2, input[ic].tangent, uv2));
